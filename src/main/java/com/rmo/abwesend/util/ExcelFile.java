...
/**
 * Das Excel-File in dem die Spieldaten gespeichert sind.
 * @author Ruedi
 *
 */
...
//	private SwissTennisLesen myParent;
	...
	public int openFile(String fileName, JTextArea message) {
		...
		inputFileName = Config.get(Config.spielplanDirKey) + "/" + fileName;
		try {
...
//	    thread.start();
	...
	public void readLine(Row lRow) {	
		...
		if (lRow.getRowNum() < 2) {
			//TODO prüfen, ob die richtigen Zeilen (erste 2 Zeilen mit Titel)
		}
		else {
			if (readRow(lRow)) {
				addMatches();
				...
				Trace.println(3, "Unglültiger Wert in Tabelle");
				Trace.println(1, "Ende readExcelFile() mit Fehler");
	...
	private void addMatches() {
		...
		try {
			if (spielerName1 != null) {
				namen = spielerName1.split(Config.get(Config.spielplanTrennCharKey));
				addMatches(namen);
			}
			if (spielerName2 != null) {
				namen = spielerName2.split(Config.get(Config.spielplanTrennCharKey));
				addMatches(namen);
			}
			spielerName1 = null;
			spielerName2 = null;	
...
//			Trace.println(3, Config.get(Config.spielplanTrennCharKey) + " " + ex.getMessage());
		}
	}
	...
	private void addMatchOfSpieler(String name, String spielTyp) throws Exception {
		...
		if (spielerId >= 0) {
			...
			match.setDatum(Config.sdfDb.format(mDatum));
