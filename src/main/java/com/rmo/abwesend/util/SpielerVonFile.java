 ...
 *
 */
public class SpielerVonFile {
	
	private boolean fehler = false;
	
	public SpielerVonFile () {
	 ...
	 * @throws Exception
	 */
	public boolean startSpielerEinlesen(String dirName, String fileName, String tableauName) throws Exception {
		...
		List<Tableau> tableauList2 = new ArrayList<Tableau>();
		...
		while ((line = bufferedReader.readLine()) != null) {
			if (line.contains(Config.get(Config.spielerSplitCharKey))) {
				String[] lSpielerZeile = line.split(Config.get(Config.spielerSplitCharKey));
				if (lSpielerZeile.length >= 0) {
					try {
						int spielerId = addSpieler(lSpielerZeile[1], lSpielerZeile[2]);
						
						int tableauId = -1;
						if (tableauName.length() > 0) {
							if (lSpielerZeile[0].contains(tableauName)) {
								tableauId = getTableauId(lSpielerZeile[0], tableauList2);	
								if (tableauId < 0) {
									fehler = true;
								}
							}
						...
						if (lSpielerZeile.length >= 4) {
							if ((lSpielerZeile[3].length() > 0) && (lSpielerZeile[4].length() > 0)) {
								spielerId = addSpieler(lSpielerZeile[3], lSpielerZeile[4]);
							}
							if (tableauId >= 0) {
								addTableau(spielerId, tableauId);
							}					
				...
				Trace.println(5, "In Zeile '" + line + "' Split-Char '" + Config.get(Config.spielerSplitCharKey) + "' nicht vorhanden"); 
			}
		}
	...
	private int addSpieler(String name, String vorname) throws Exception {
		...
		Trace.println(5, "SpielerVonFile.addSpieler: " + spieler.getName() + " " + spieler.getVorName());
