...
public class TableauVerwalten extends BasePane {
	...
	public JPanel getPanel() {
		...
		panel.add(addButtons(), BorderLayout.PAGE_START);
	...
	private JComponent addButtons() {
		...
		btnAendern.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				addPosition.setText((String)tableauTableData.getValueAt(row, 2));
				addKonkurrenz.setText((String)tableauTableData.getValueAt(row, 3));
		    }		
		});
...
//			this.addTableModelListener(new MyListener());
		...
		public void readAllData() {
			try {
				...
				tableauListe = new ArrayList<Tableau>();	// statt remove
		...
		public void saveAll() {
			...
			try {
				for (int i = 0; i < tableauListe.size(); i++) {
					TableauData.instance().add(tableauListe.get(i));
				}
			}
			catch (Exception ex) {
