...
public class KalenderAbwesend extends KalenderBase {
	...
	/**
	 * Konstruktor, initialisiert den Grid, setzt den Header.
	 * Version 1 ist der Header in der Liste (scrolllt mit)
	 * Version 2 für grosse Liste, Header stabil)
	 */
...
//			showDeleteButton(rowNr);
	...
	private void showRectangle(Spieler pSpieler, int rowNr) {
		kalenderPanel.add(new JLabel(pSpieler.getName() + " " +  pSpieler.getVorName().charAt(0)), getConstraintFirst(0, rowNr));	
	...
	private void showNextRect(int colNr, int posStart, boolean isWeekend, String abwTime, String matches) {
		...
		if (posStartAbw == 0) {
			if (getPosEndAbw(abwTime, isWeekend) == 0) {
				// wenn kein Eintrag
				posStartAbw = RECT_WIDTH + 10;
				posEnd = RECT_WIDTH + 10;
			}
		}	
		...
		if (posStartAbw < posStartMatch) {
			...
			if (posStartMatch < RECT_WIDTH) {
				...
				// hier ist der Fehler bei D19:00 posStartAbw = 60, posBis = 30
				addRect(colNr, posStartMatch, posEnd, COLOR_MATCH, rowNr);				
			}
			else {
				posEnd = RECT_WIDTH + 10;
			}
	...
	private void showGames(Spieler pSpieler, int rowNr) {
		...
		if (spieleAnzeigen) {
			...
			for (int i = posVon; i < posBis; i++) {
				kalenderPanel.add(new JLabel(pSpieler.getSpielAt(i)), getConstraintNext(i-Config.showBeginNumber+1, rowNr));
			}
		}
	}
	...
	public void addInputFields(Spieler pSpieler) {
		...
		oldTextList = new ArrayList<String>(Config.turnierMaxTage);
		inputTextList = new ArrayList<JTextField>(Config.turnierMaxTage);
		int colNr = 1;
		for (int i = 0; i < Config.turnierMaxTage; i++) {
			oldTextList.add(pSpieler.getAbwesendAt(i));
			
			JTextField tagText = new JTextField();
			tagText.setSize(RECT_WIDTH-10,RECT_HIGTH);
	...
	private int getPosStartAbw(String abwesendZeit, double startPos, boolean isWeekend) {
		...
		if (abwesendZeit.endsWith("-")) {
			double pos = getPos(abwesendZeit.substring(0, abwesendZeit.length()-1), isWeekend);
			if (pos >= startPos) {
				return (int) pos;
			}
		}
	...
	private int getPosEndAbw(String abwesendZeit, boolean isWeekend) {
		...
		if (abwesendZeit.startsWith("-")) {
			return (int) getPos(abwesendZeit.substring(1, abwesendZeit.length()), isWeekend);
		}
		if (abwesendZeit.endsWith("-")) {
			return RECT_WIDTH;
		}
	...
	/**
	 * Die Position des Matches berechnen, wenn kein Match, dann wir grosse Zahl zurückgegeben
	 * @param abwesendZeit
	 * @param startPos
	 * @return
	 */
	...
	/**
	 * Die Zeit als Nummer, es werden nur die vollen Stunden berücksichtigt.
	 * Wenn keine gültige zeit dann -1.
	 * Wenn zeit ausserhalb Bereich, dann 0
	 * 
	 * @param zeit
	 * @return die Zeit als Nummer
	 */
	private double getPos(String zeit, boolean isWeekend) {
