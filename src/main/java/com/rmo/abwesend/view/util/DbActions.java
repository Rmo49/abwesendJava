...
//				System.out.println("tableau: focus gained");
		...
		/*
		 * pane.add(new JLabel(" "), getConstraintNext(1, zeileNr++)); JLabel
		 * labelTitel5 = new
		 * JLabel("Datei mit DB-Passwort generieren, DbPassword.txt wird generiert");
		 * labelTitel5.setFont(Config.fontTitel); pane.add(labelTitel5,
		 * getConstraintNext(1, zeileNr++));
		 * 
		 * JTextArea labelText2 = new JTextArea(1,0);
		 * labelText2.append("Unten das neue Passwort eingeben");
		 * labelText2.setEditable(false); pane.add(labelText2, getConstraintNext(1,
		 * zeileNr++));
		 * 
		 * dbPassword = new JTextField(); dbPassword.setPreferredSize(new Dimension(150,
		 * Config.textFieldHeigth)); dbPassword.setEditable(true); pane.add(dbPassword,
		 * getConstraintNext(1, zeileNr++));
		 * 
		 * JButton btnPwGenerate = new JButton("Passwort generieren");
		 * btnPwGenerate.addActionListener(new ActionListener() {
		 * 
		 * @Override public void actionPerformed(ActionEvent e) { startPwGenerate(); }
	 ...
	 */
	/*
	 * private void startPwGenerate() { if (dbPassword.getText().length() < 3) {
	 * CmUtil.alertWarning("Passwort generieren", "Passwort zu kurz"); return; }
	 ...
	 * file.savePw(pwHash);
	 * 
	 * CmUtil.alertWarning("Passwort gespeichert", "siehe: " +
	 * Config.sDbPwFileName); }
	 */
}
